# Emir Can Onel

**Product-oriented full stack engineer.** I build SaaS platforms, mobile apps, and AI-powered tools — from first commit to production.

Currently working on **Algonel** (multi-tenant stock & sales management SaaS) and **Covera** (insurance operations platform).

---

### What I'm building

**[Algonel Admin Panel](https://emircanonel.vercel.app/en/work/ima-admin)** — Multi-tenant back-office system covering tenants, branches, warehouses, products, variants and audit workflows. Next.js + Supabase, multi-tenant data isolation.

**[Algonel Mobile App](https://emircanonel.vercel.app/en/work/ima-app)** — Branch operations app for Algonel. Orders, stock transfers, customer checkout, realtime activity. Expo + Supabase.

**[Covera](https://emircanonel.vercel.app/en/work/covera)** — Insurance operations platform. Customers, policies, collections, dashboards and RBAC with row-level security. Next.js + Supabase.

**[Mercan.ai](https://emircanonel.vercel.app/en/work/mercan-ai)** — AI video clipping platform. Next.js dashboard + FastAPI backend for job queues, render options, subtitles and short-form clip delivery.

---

### Stack

![TypeScript](https://img.shields.io/badge/TypeScript-000?style=flat-square&logo=typescript)
![React](https://img.shields.io/badge/React-000?style=flat-square&logo=react)
![Next.js](https://img.shields.io/badge/Next.js-000?style=flat-square&logo=nextdotjs)
![React Native](https://img.shields.io/badge/React_Native-000?style=flat-square&logo=react)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-000?style=flat-square&logo=tailwindcss)
![Three.js](https://img.shields.io/badge/Three.js-000?style=flat-square&logo=threedotjs)
![Node.js](https://img.shields.io/badge/Node.js-000?style=flat-square&logo=nodedotjs)
![FastAPI](https://img.shields.io/badge/FastAPI-000?style=flat-square&logo=fastapi)
![Supabase](https://img.shields.io/badge/Supabase-000?style=flat-square&logo=supabase)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-000?style=flat-square&logo=postgresql)
![MongoDB](https://img.shields.io/badge/MongoDB-000?style=flat-square&logo=mongodb)
![Python](https://img.shields.io/badge/Python-000?style=flat-square&logo=python)
![OpenAI](https://img.shields.io/badge/OpenAI-000?style=flat-square&logo=openai)

---

### How I work

- Multi-tenant architecture from day one, not bolted on later
- Mobile and web from a single product vision
- AI as a layer in the product, not the whole point
- Ship early, iterate fast, don't gold-plate

---

### Stats

[![Streak](https://github-readme-streak-stats.herokuapp.com/?user=j4ckcaver&hide_border=true&theme=transparent)](https://github.com/j4ckcaver)

<p>
  <img height="160" src="http://github-profile-summary-cards.vercel.app/api/cards/stats?username=j4ckcaver&theme=transparent" />
  <img height="160" src="http://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=j4ckcaver&theme=transparent&utcOffset=3" />
</p>

[![Profile details](http://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=j4ckcaver&theme=transparent)](https://github.com/j4ckcaver)

[![](https://raw.githubusercontent.com/j4ckcaver/j4ckcaver/main/profile-3d-contrib/profile-night-rainbow.svg)](https://github.com/j4ckcaver)

---

### Find me

[Portfolio](https://emircanonel.vercel.app) · [LinkedIn](https://linkedin.com/in/emircanonel)

<img src="https://komarev.com/ghpvc/?username=j4ckcaver&color=0e75b6&style=flat&label=views" />